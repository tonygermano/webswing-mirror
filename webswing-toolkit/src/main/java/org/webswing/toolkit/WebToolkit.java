...
public class WebToolkit extends SunToolkit {
    ...
    private ServerJvmConnection jmsConnector;
    ...
    public void doSendPaint(BufferedImage bi) {
        Map<String, BufferedImage> pm = new HashMap<String, BufferedImage>();
        Map<String, JsonWindowInfo> wi = new HashMap<String, JsonWindowInfo>();
        pm.put("test", bi);
        wi.put("test", new JsonWindowInfo());
        Map<String, String> paintB64Strings = jmsConnector.createEncodedPaintMap(pm);
        jmsConnector.sendJsonObject(new JsonPaintRequest(paintB64Strings, wi));
    }

    public void setJmsConnector(ServerJvmConnection jmsConnector) {
