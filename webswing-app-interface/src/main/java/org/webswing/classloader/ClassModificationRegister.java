	...
	public ClassModificationRegister() {
		...
		try {
			if (acquireLock()) {
				...
				Logger.info("ClassModificationRegister: lock acquired.");
			} else {
			}
		} catch (IOException ex) {
			Logger.error("ClassModificationRegister: Failed to create unmodified class register map. " + ex.getMessage());
		}
	}
