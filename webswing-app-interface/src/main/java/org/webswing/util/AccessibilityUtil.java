...
//		accessibleRoleMap.put(AccessibleRole.OPTION_PANE, "dialog"); option pane is included in a dialog
		accessibleRoleMap.put(AccessibleRole.FRAME, "dialog"); // TODO map to dialog?
		accessibleRoleMap.put(AccessibleRole.WINDOW, "dialog"); // TODO map to dialog?
	...
	public static AccessibilityMsg getAccessibilityInfo(Component c, int x, int y) {
	...
	private static AccessibilityMsg getAccessibilityInfo(Accessible accessible) {
		...
		List<AccessibilityHierarchyMsg> hierarchy = new ArrayList<>();
	...
	private static void resolveHierarchy(Accessible a, List<AccessibilityHierarchyMsg> hierarchy) {
		...
		if (role != null && allowedHierarchyRoles.contains(role) && (StringUtils.isNotBlank(text) || allowedNoTextHierarchyRoles.contains(role))) {
			AccessibilityHierarchyMsg msg = new AccessibilityHierarchyMsg(id, role, text);
	...
	private static void handleComponent(AccessibilityMsg result, Component a) {
		Component c = (Component) a;
		try {
