...
public class LoginHandlerImpl extends AbstractUrlHandler implements LoginHandler {
	private final WebswingSecurityProvider configProvider;

	public LoginHandlerImpl(UrlHandler parent, SecurityManagerService securityManger, WebswingSecurityProvider configProvider) {
		super(parent);
	...
	protected void login(HttpServletRequest req, HttpServletResponse resp, WebswingAuthenticationException e) throws ServletException, IOException {
		...
		if (user != null) {
			...
			WebswingCredentials credentials = configProvider.get().getCredentials(req, resp, e);
			if (credentials != null) {
				try {
					WebswingUser resolvedUser =  configProvider.get().getUser(credentials);
