	...
	public void init() {
		...
		try {
			...
			try {
				...
				SPConfig spConfig = new SPConfig(new ByteArrayInputStream(spTemplate.getBytes(StandardCharsets.UTF_8)));
	...
	public Saml2Credentials getCredentials(HttpServletRequest request, HttpServletResponse response, WebswingAuthenticationException e) throws ServletException, IOException {
		...
		if (StringUtils.isEmpty(samlResponse)) {
			...
			try {
				...
				return new Saml2Credentials(samlResponse, user);
			} catch (SAMLException e1) {
				new IOException("Failed to auhenticate.", e1);
			}
		}
		return null;
	}

	public WebswingUser getUser(Saml2Credentials credentials) throws WebswingAuthenticationException {
